...
CubicMotion.prototype = {
	...
	getPosition : function() {
		...
		return ((this._path[0] * a + this._path[1]) * a + this._path[2]) * a
				+ this._path[3]
	}
};
function Motion(a) {
...
Motion.prototype = {
	...
	setTarget : function(b) {
		...
		a /= this._duration;
		if (a >= 0 && a < 1) {
			c = (((5 * this._path[0] * a + 4 * this._path[1]) * a + 3 * this._path[2])
					* a + 2 * this._path[3])
					* a + this._path[4];
			d = ((20 * this._path[0] * a + 12 * this._path[1]) * a + 6 * this._path[2])
					* a + 2 * this._path[3]
		}
		this._target = b;
		b -= e;
		this._path = [ -d / 2 - c * 3 + b * 6, d * 3 / 2 + c * 8 - b * 15,
				-d * 3 / 2 - c * 6 + b * 10, d / 2, c, e ]
	},
	getPosition : function() {
		var a = (new Date() - this._date) / this._duration;
		if (a < 0 || a >= 1) {
...
var UI = new function() {
	...
	this.init = function() {
		b();
		function e(h) {
			var f = "url(base" + Control.config.skin + ".png)";
...
var Game = new function() {
	...
	function H(i, O, N, M) {
		...
		var L = 30;
		return {
			x : N * Math.cos((2 * Math.PI) * ((i - M) / 15 - 1 / 4)) * (O + L)
					/ L,
			y : 200 - O * 20 * (O / 2 + 60) / 60 - 0.3 * N
					* Math.sin((2 * Math.PI) * ((i - M) / 15 - 1 / 4))
		}
	}
	CanvasRenderingContext2D.prototype.drawFront = function(S, R, i, L, N, M,
			P, O) {
		this.beginPath();
		var Q = [ H(S - 0.015, R + 0.015, i, L), H(S + 1.015, R + 0.015, i, L),
				H(S + 1.015, R - 1.015, i, L), H(S - 0.015, R - 1.015, i, L) ];
	...
	CanvasRenderingContext2D.prototype.drawTop = function(L, Q, M, i, N, P) {
		var O = [ H(L - 0.015, Q, false, M), H(L - 0.015, Q, true, M),
				H(L + 1.015, Q, true, M), H(L + 1.015, Q, false, M) ];
		this.beginPath();
	...
	CanvasRenderingContext2D.prototype.drawSide = function(L, P, M, i, N) {
		var O = [ H(L, P - 1.015, false, M), H(L, P + 0.015, false, M),
				H(L, P + 0.015, true, M), H(L, P - 1.015, true, M) ];
		this.beginPath();
		this.moveTo(O[0].x, O[0].y);
		this.lineTo(O[1].x, O[1].y);
		this.lineTo(O[2].x, O[2].y);
	...
	function B(Q, P) {
		...
		var L = (A.mode == 3 ? 1000
				: (20 + 2980 * Math.exp(-Game.lines / 35)) >> 0);
		if (u.down) {
			Game.score += x / 100;
			if (A.mode != 3) {
		...
		if (P || (u.left ^ u.right) && (M - h > 150)) {
			h = M;
			Game.move(P == "left" ? -1 : P == "right" ? 1 : u.left ? -1
					: u.right ? 1 : 0)
		}
		slotY = Math.floor(n);
		if (p.length == 1) {
	...
	function r(M, x, N) {
		...
		switch (M) {
		case 0:
			return "rgb(" + ((x * 255) >> 0) + "," + ((x * i) >> 0) + ","
					+ ((x * i) >> 0) + ")";
		case 1:
			return "rgb(" + ((x * i) >> 0) + "," + ((x * 255) >> 0) + ","
					+ ((x * i) >> 0) + ")";
		case 2:
			return "rgb(" + ((x * i) >> 0) + "," + ((x * i) >> 0) + ","
					+ ((x * 255) >> 0) + ")";
		case 3:
			return "rgb(" + ((x * 255) >> 0) + "," + ((x * 255) >> 0) + ","
					+ ((x * i) >> 0) + ")";
		case 4:
			return "rgb(" + ((x * 255) >> 0) + "," + ((x * i) >> 0) + ","
					+ ((x * 255) >> 0) + ")";
		case 5:
			return "rgb(" + ((x * i) >> 0) + "," + ((x * 255) >> 0) + ","
					+ ((x * 255) >> 0) + ")";
		case 6:
			return "rgb(" + ((x * 220) >> 0) + "," + ((x * 220) >> 0) + ","
					+ ((x * 220) >> 0) + ")";
		case 20:
			return "rgb(" + ((x * 100) >> 0) + "," + ((x * 100) >> 0) + ","
					+ ((x * 100) >> 0) + ")";
		case 30:
			return "rgb(" + ((x * 190) >> 0) + "," + ((x * 190) >> 0) + ","
					+ ((x * 0) >> 0) + ")";
		default:
			return "rgb(" + ((x * 220) >> 0) + "," + ((x * 220) >> 0) + ","
					+ ((x * 220) >> 0) + ")"
	...
	this.drawCylinder = function(Q, N, P, ad) {
		...
		for (aa = 0; aa < 15; aa++) {
			...
			for (S = 0; S < 15; S++) {
				...
				if (b[U][S]) {
					...
					if (P && ad.contains(S)) {
						Y++;
						E.globalAlpha = Math.max(0, Math.min(1, O
								* (1 - P * 1.5)));
						X = S
					} else {
						if (A.mode == 3) {
					...
					O *= 1 + Math.sin(U * 77777 + L / 871)
							* Math.sin(U * 31247 + L / 1713)
							* Math.sin(S * 41996 + L / 1713)
							* Math.sin(S * 85555 + L / 797) / 3;
					O = O < 0 ? 0 : O > 1 ? 1 : O;
					switch (ac) {
					case 0:
						E.fillStyle = "rgb(" + ((O * 255) >> 0) + ","
								+ ((O * 0) >> 0) + "," + ((O * 0) >> 0) + ")";
						break;
					case 1:
						E.fillStyle = "rgb(" + ((O * 0) >> 0) + ","
								+ ((O * 255) >> 0) + "," + ((O * 0) >> 0) + ")";
						break;
					case 2:
						E.fillStyle = "rgb(" + ((O * 0) >> 0) + ","
					...
					case 5:
						...
						break;
					case 6:
						E.fillStyle = "rgb(" + ((O * 220) >> 0) + ","
								+ ((O * 220) >> 0) + "," + ((O * 220) >> 0)
								+ ")";
						break;
					case 20:
						E.fillStyle = "rgb(" + ((O * 100) >> 0) + ","
								+ ((O * 100) >> 0) + "," + ((O * 100) >> 0)
								+ ")";
						break;
					case 30:
						E.fillStyle = "rgb(" + ((O * 190) >> 0) + ","
								+ ((O * 190) >> 0) + "," + ((O * 0) >> 0) + ")";
						break;
					default:
						E.fillStyle = "rgb(" + ((O * 220) >> 0) + ","
					...
					E
							.drawFront(
									...
									R,
									(ab > 11.9 || ab < 10.9)
											&& ((V = b[(U + 14) % 15][S]) && V[1] == b[U][S][1]),
									(ab < 2.1 || ab > 3.1)
											&& ((V = b[(U + 1) % 15][S]) && V[1] == b[U][S][1]),
		...
		for (aa = 0; aa < 15 && !N; aa++) {
			...
			for (Z = 1; Z < 5; Z++) {
				O = Math
						.max(O, W[(aa + Z) % 15] - Z, W[(aa - Z + 15) % 15] - Z)
			}
			if (O > 9) {
				E.globalAlpha = (O - 9) / 5;
				E.fillStyle = "rgb("
	...
	this.clear = function() {
		...
		if (A.mode == 2) {
			var N = b;
			N[0][0] = N[3][0] = N[6][0] = N[9][0] = N[12][0] = [
					(Math.random() * 6 + 1) >> 0, 0 ];
			k = 1
		}
		if (A.mode == 3) {
...
function Control() {
	...
	g("high_form").onsubmit = function() {
		...
		for ( var f = 2; f >= 0; f--) {
			if (Game.mode < 3 && h > c[Game.mode - 1][f][0] || Game.mode == 3
					&& h < c[2][f][0]) {
				if (f < 2) {
		...
		d();
		b();
		g("sorryText").innerHTML = '<div style="padding-left:60px;">'
				+ e(Game.mode - 1) + "</div>";
		return false
	};
